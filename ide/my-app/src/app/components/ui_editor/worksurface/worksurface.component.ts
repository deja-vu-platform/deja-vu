...
const widgetContainerMaker = WidgetContainer();

const WIDGET_WS_REF = 'widget-work-surface';
@Component({
  selector: 'dv-worksurface',
  templateUrl: './worksurface.component.html',
  styleUrls: ['./worksurface.component.css']
})
export class WorkSufaceComponent {
  @Input() currentZoom: number;

  containerRef = widgetContainerMaker.getContainerRef();

  // make it such that it takes in a component and a widget Id instead of the widget straight

  private getWorkSurfaceRef() {
    return WIDGET_WS_REF;
  }

  createWorkSurface(outerWidgetId, height, width) {
    const workSurface = $('<div></div>');
  ...
  createOrResetWorkSurface(outerWidget, zoom, editable) {
    const widgetId = outerWidget.meta.id;
    let workSurface = $('#' + WIDGET_WS_REF + '_' + widgetId);
    if (workSurface.length === 0) {
      workSurface = this.setUpEmptyWorkSurface(outerWidget, zoom, editable);
    } else {
  ...
  private resetWorkSurface(workSurface) {
    const state = {
      zoom: 1,
    };
  ...
  private makeRecursiveWidgetContainersAndDisplay(innerWidget, outerWidget,

...
const importedConcepts: { [name: string]: Object} = {
  allocator,
  authentication,
  authorization,
  comment,
...
// each imported Concept Module has an Angular Module
...
const modules: any[] = Object.values(importedConcepts)
  .map(getNgModule);

// create Concept Definitions from imported Concept Modules
importedConcepts['dv'] = dv;
...
function isConceptComponent(f: any) {
...
function conceptDefinitionFromModule(
  ...
  return {
    ...
    components: Object.values(importedModule)
      .filter(isConceptComponent)
      .map((component): ConceptComponentDefinition => {
...
export const conceptDefinitions = _
  .map(importedConcepts, conceptDefinitionFromModule);

const dvConceptIdx = conceptDefinitions.findIndex((cd) => cd.name === 'dv');
const [dvConcept] = conceptDefinitions.splice(dvConceptIdx, 1);
// TODO: filter components in dvConcept

App.conceptDefinitions = conceptDefinitions;
App.dvConcept = dvConcept;

export const dvCoreComponents = dvConcept.components
  .map(({ component: c }) => <any>c);

dvConcept.components.push(({
  name: 'text',

...
export class AppActionInstance extends ActionInstance {
  ...
  readonly referenced: { [actionID: string]: string[] } = {};
  ...
  resolveReferences() {
    ...
    const ids = new Set(
      [this.id, ..._.map(this.of.getChildren(true), (c) => c.id)]
    );
    ...
    this.of.outputSettings.forEach(({ name, value }) => {
      this.resolveOneReference(this, name, value);
    });
  }
  ...
  private resolveOneReference (
    ...
    inInput?: InInput
      ...
      if (resolution) {
        this.referenced[resolution.fromAction.id].push(resolution.ioName);
      }
  }
  ...
  private resolveExpression(
    ...
    if (expr.startsWith('$')) {
      ...
      } else if (this.of.inputs.includes(ioName)) {
      ...
      const maybeFromAction = this.of.findChild(clicheN, actionN);

...
export class ShowUnassignedTasksComponent implements AfterInit {
  ...
  private fetch() {
    if (this.fetched !== this.assigner.atom_id) {
      ...
      .get(`
        unassignedTasks(assigner_id: "${this.assigner.atom_id}") {
          name, atom_id, expiration_date
        }
      `)
       .map(data => data.unassignedTasks)
